...
var printMenu = function(foodList) {
  ...
  _.map(foodList, function(typeItems, type) {
    console.log(type);
    _.each(typeItems, function(f) {
      console.log(_.template("{{ name }} ... ${{ price }}")(_.pick(f, 'name', 'price')));
    });
  });
};
...
var query5 = _.filter(bakedGoods, function(item){ return _.propertyOf(item)('price') > 4; });
